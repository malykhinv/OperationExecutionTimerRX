...
import com.malykhinv.operationsexecutiontimerrx.model.CalculationModel;
...
@Module
public class CollectionsOperator implements CalculationModel<List<Byte>> {
    ...
    @Override
    public List<IndexedData<Integer, List<Byte>>> getIndexedData() {
        List<IndexedData<Integer, List<Byte>>> indexedCollections = new ArrayList<>();
    ...
    @Override
    public IndexedData<Integer, List<Byte>> fillIndexedData(IndexedData<Integer, List<Byte>> indexedCollection, int size) {
        Log.d(TAG, "fill indexed collection: " + Thread.currentThread().getName());
        List<Byte> collection = indexedCollection.getData();
        collection.addAll(Collections.nCopies(size, (byte) 1));
        return indexedCollection;
    }

    @Override
    public OperationResult operateWithData(IndexedData<Integer, List<Byte>> indexedCollection) {
